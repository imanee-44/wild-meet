...
export function OutingCard({ outing }: { outing: Outing }) {
  ...
  const pct = Math.min(100, Math.round((outing.spotsTaken / outing.spotsTotal) * 100));
  ...
  return (
    ...
    >
      <div className="relative aspect-[5/4] overflow-hidden">
        ...
        <span
          className={`absolute right-4 top-4 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] ${categoryStyles[outing.category]}`}
        >
          {outing.category}
        </span>
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        ...
        <div className="mt-auto space-y-3">
          ...
          <div className="h-1 w-full overflow-hidden rounded-full bg-muted">
            <div
              className={`h-full transition-all duration-500 ${full ? "bg-destructive" : "bg-terracotta"}`}
              style={{ width: `${pct}%` }}
            />
          </div>
