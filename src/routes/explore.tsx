...
export const Route = createFileRoute("/explore")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Parcours les sorties nature à venir au Maroc : camping, plage, montagne. Filtre par terrain et par date." },
...
const filters: { id: Filter; label: string; icon: typeof Tent | null }[] = [
                ...
                >
                  ...
                  <span
                    className={`ml-1 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
                      isActive ? "bg-cream/20 text-cream" : "bg-muted text-muted-foreground"
                    }`}
                  >
        ...
        {filtered.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border bg-card/40 p-16 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Essaie une autre combinaison, ou organise la sortie qui te manque.
            </p>
          </div>
        ) : (
          <>
...
function FeaturedCard({ o }: { o: typeof outings[number] }) {
  ...
  return (
    <a
      href={`/outings/${o.id}`}
      className="group relative flex min-h-[420px] flex-col justify-end overflow-hidden rounded-3xl text-cream shadow-lift"
    >
      <img
        src={o.image}
        alt={o.title}
